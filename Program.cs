...
#nullable disable
        ...
        static private int VerbCreate(CreateOptions opts)
        {
            int lResult = 0;
            WriteVersion();
            WorkingDir = KnxProdHelper.GetAbsWorkingDir(opts.XmlFileName);
            ...
            if (lSuccess)
            {
                string lEtsPath = KnxProdHelper.FindEtsPath(lInclude.GetNamespace());
                lResult = KnxProdHelper.ExportKnxprod(lEtsPath, WorkingDir, lOutputFileName, lTempXmlFileName, lInclude.BaggagesName, opts.XsdFileName, opts.Debug, !opts.NoXsd);
            }
            else
                lResult = 1;
            if (lResult > 0)
        ...
        static private int VerbCheck(CheckOptions opts)
        {
            WriteVersion();
            CommentDebug = opts.CommentDebug;
            string lWorkingDir = KnxProdHelper.GetAbsWorkingDir(opts.XmlFileName);
            ...
            if (opts.Debug) Console.WriteLine("Writing debug file to {0}", lTempXmlFileName);
            lXml.Save(lTempXmlFileName);
            lSuccess = KnxProdHelper.ValidateXsd(lWorkingDir, lTempXmlFileName, opts.XmlFileName, opts.XsdFileName, !opts.NoXsd) || lSuccess;
            return lSuccess ? 1 : 0;
        }
        ...
        static private int VerbKnxprod(KnxprodOptions opts)
        {
            ...
            string lEtsPath = KnxProdHelper.FindEtsPath(match.Groups[1].Value);
            return KnxProdHelper.ExportKnxprod(lEtsPath, lWorkingDir, lOutputFileName, opts.XmlFileName, Path.GetFileName(opts.XmlFileName).Replace(".xml", ".baggages"), opts.XsdFileName, false, !opts.NoXsd);
        }
        ...
        static private int VerbBaggages(BaggagesOptions opts)
        {
            ...
            string lWorkingDir = KnxProdHelper.GetAbsWorkingDir(opts.BaggagesDir);
